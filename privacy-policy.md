...
| Discord Guild (Server) ID | Keep data scoped per-server | AI chat history, warnings, reminders |
| Discord Channel ID | Deliver reminders to the correct channel | `/remind` |
...
| Moderation records (warnings, reason, moderator tag) | Track moderation history per server | `/mod warn` |
...
- Conversation history, warnings, reminders, and economy data are stored in our database, scoped to your Discord User ID and the Guild ID where the interaction happened.
...
- **Server administrators** can request removal of moderation records or all stored data for their server by contacting us (see Section 7).
